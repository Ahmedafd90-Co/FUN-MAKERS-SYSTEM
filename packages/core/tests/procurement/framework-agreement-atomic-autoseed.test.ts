...
describe('FrameworkAgreement atomic create+autoSeed (PIC-80)', () => {
  ...
  beforeAll(async () => {
    assertTestDb();
    testUser = await prisma.user.create({ data: { orgId: SINGLETON_ORG_ID, email: `${ts}@test.com`, name: 'FA Atomic User', passwordHash: 'test-hash', status: 'active' } });
    ...
    testEntity = await prisma.entity.create({ data: { orgId: SINGLETON_ORG_ID, code: `ENT-FAA-${ts}`, name: 'FA Atomic Entity', type: 'parent', status: 'active' } });
    testVendor = await prisma.vendor.create({ data: { orgId: SINGLETON_ORG_ID, entityId: testEntity.id, vendorCode: `VEN-FAA-${ts}`, name: `FA Atomic Vendor ${ts}`, status: 'active', createdBy: testUser.id } });
    await prisma.currency.upsert({ where: { code: 'SAR' }, create: { code: 'SAR', name: 'Saudi Riyal', symbol: 'SR', decimalPlaces: 2 }, update: {} });
    testProject = await prisma.project.create({ data: { orgId: SINGLETON_ORG_ID, code: `PROJ-FAA-${ts}`, name: 'FA Atomic Project', entityId: testEntity.id, currencyCode: 'SAR', startDate: new Date(), createdBy: testUser.id, status: 'active' } });
