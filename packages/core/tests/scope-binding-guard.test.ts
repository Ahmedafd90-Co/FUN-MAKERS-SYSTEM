...
const EXEMPTIONS: Exemption[] = [
  ...
  { file: 'commercial/revised-contract-value.ts', fn: 'getRevisedContractValue',
    category: 'SAFE',
    reason: 'findUnique({where:{id: projectId}}) on Project — id IS the projectProcedure-validated scope (called from projects.get under projectProcedure); assert would be tautology (PIC-104; same shape as getProject/getFinancialKpis/getCommercialDashboard). getApprovedVariationDelta uses a where-filter aggregate (not a by-id read), so it is not enumerated.' },
