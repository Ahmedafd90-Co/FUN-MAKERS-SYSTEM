...
/**
 * PIC-99 PR-2: every getMonthlyCostSheet call requires expectedOrgId. These
 * computational tests scope by projectIds (per-test fixture); we pass the
 * F2 singleton orgId because the seeded projects + entity below rely on
 * Project.orgId @default singleton. Three-tier route resolution
...
describe('getMonthlyCostSheet — single project', () => {
  it('defaults to trailing 12 months ending at reportMonth', async () => {
    const s = await getMonthlyCostSheet({
      expectedOrgId: SINGLETON_ORG_ID,
