 ...
 *   - PIC-99 PR-1 soft-delete seam: the service filters IpaForecast rows
 *     by `deletedAt: null` so soft-deleted forecast plans drop out of the
 *     report (audit trail preserved at the DB layer; planning view honest).
 *
 * Multi-tenancy (PIC-99 PR-2 / PD ruling cfc36eab 2a):
 *   - `expectedOrgId: string | null` is REQUIRED on options. The service
 *     applies `where.orgId = expectedOrgId` on the project.findMany when
 *     non-null. The route passes `isPlatformAdmin ? null : ctx.orgId`.
 *     This is defense-in-depth alongside the route-layer three-tier
 *     resolution: even if a future caller skips the route gate, the
 *     service refuses to walk projects outside the supplied org.
 *
...
export interface MonthlyCostSheetOptions {
  /**
   * REQUIRED scope filter. Pass `ctx.orgId` for tenant-bounded callers and
   * `null` ONLY for platform-admin (system.admin) cross-org operators.
   * When non-null, `prisma.project.findMany` filters `where.orgId = X` so
   * project loading is intrinsically tenant-bounded — independent of any
   * route-level filtering.
   *
   * Required (not optional) by design so callers cannot accidentally elide
   * the org scope. PD ruling cfc36eab 2a — route + service defense-in-depth.
   */
  expectedOrgId: string | null;
  /** Optional explicit list of projects. If omitted the service returns
   *  every project that matches `expectedOrgId` (or all if null). The route
   *  is responsible for permission-filtering BEFORE calling the service. */
  projectIds?: string[];
