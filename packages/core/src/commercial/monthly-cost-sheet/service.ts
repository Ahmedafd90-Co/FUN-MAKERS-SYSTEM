 ...
 *   2. Resolves the three-tier scope: platform-admin (cross-org),
 *      PMO-in-tenant (own-org), or assigned-only.
 *
 ...
 *     total_claimed).
 *
 * PIC-99 PR-1 soft-delete seam:
 *   - `prisma.ipaForecast.findMany` filters `deletedAt: null`. Soft-deleted
 *     forecasts (PR-1's audit-preserving delete pattern) drop from the
 *     report. Audit trail stays in the DB; planning view stays honest.
 *
 * PIC-99 PR-2 multi-tenancy defense-in-depth (PD ruling cfc36eab 2a):
 *   - `options.expectedOrgId` is REQUIRED on the public entry point. When
 *     non-null, `prisma.project.findMany` applies `where.orgId = X`. The
 *     route layer passes `isPlatformAdmin ? null : ctx.orgId`. Even if a
 *     future caller forgets the route gate, this service refuses to walk
 *     projects outside the supplied org.
 *   - explicit `projectIds` still bypasses the org filter at the SERVICE
 *     layer (it can be set when org filter is null too) — the route layer
 *     is responsible for intersecting explicit lists with the org-scoped
 *     project set. Service stays simple: org filter applied iff supplied.
 */
...
export async function getMonthlyCostSheet(
  ...
  // Load projects.
  // PIC-99 PR-2 multi-tenancy (PD ruling cfc36eab 2a): when expectedOrgId
  // is non-null, the project list is intrinsically bounded to that org.
  // The route layer additionally permission-filters / intersects explicit
  // project lists with the org-scoped set — but if the route is ever
  // bypassed, the service still refuses to walk other tenants' projects.
  const where: Prisma.ProjectWhereInput = {};
  if (options.expectedOrgId !== null) {
    where.orgId = options.expectedOrgId;
  }
