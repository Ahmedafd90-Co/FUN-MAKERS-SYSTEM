...
import { Plus, Calculator, ShieldOff } from 'lucide-react';
...
export default function CostProposalsListPage() {
  ...
  const { data, isLoading, error } = trpc.commercial.costProposal.list.useQuery({
    projectId,
    skip: page * pageSize,
    take: pageSize,
  ...
  return (
    <div className="space-y-4">
      <PageHeader
        ...
        actions={
          <Link href={`/projects/${projectId}/commercial/cost-proposals/create`}>
            <Button size="sm">
      ...
      {error?.data?.code === 'FORBIDDEN' ? (
        <div className="py-16 text-center space-y-2">
          ...
          <p className="text-xs text-muted-foreground">You don&apos;t have permission to view Cost Proposals in this project.</p>
        </div>
      ) : error ? (
